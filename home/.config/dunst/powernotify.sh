...
# Verifica se está no KDE Plasma — se sim, não notifica
...
if [ "$AC" -eq 1 ]; then
    ...
    MESSAGE="Performance"
    ICON="/usr/share/icons/Papirus-Light/16x16/panel/battery-100-charging-profile-performance.svg"
else
    TITLE="Conectado na bateria (${BAT}%)"
    MESSAGE="Economia de energia"
    ICON="/usr/share/icons/Papirus-Light/16x16/panel/battery-100-charging-profile-powersave.svg"
fi
